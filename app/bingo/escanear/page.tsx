...
function QRScanner({
  ...
  const startedRef = useRef(false); // true only after start() resolves successfully
...
function EscanearContent() {
  ...
  const [scanning, setScanning] = useState(false); // true while processing a scan
  ...
  // Activate scanner once card is loaded
  useEffect(() => {
    if (step === "scanner" && card) setScannerActive(true);
  }, [step, card]);
  ...
  async function handleQRDetected(raw: string) {
    ...
    // Extract token from URL like "https://example.com/bingo/escanear?t=XXXXXX"
    let token = raw;
    try {
      ...
      // raw might already be just the token
    }
      ...
      <div className="relative z-10 w-full max-w-lg space-y-4">
        ...
        {step === "codigo" && (
          <div className="bg-white/95 p-6 border-8 [border-image:linear-gradient(to_right,#bf953f,#fcf6ba,#b38728)1]">
            <h2 className="text-xl font-bold mb-1 text-[#5c4a2e]">Bingo — Escáner QR</h2>
            ...
            <button
              ...
              className="w-full py-3 bg-[#8a6d3b] text-white font-bold rounded-xl disabled:opacity-50"
            >
              {loadingCard ? "Cargando..." : "Continuar"}
            </button>
            {cardError && (
          ...
          <>
            ...
            <div className="bg-white/95 px-5 py-4 border-8 [border-image:linear-gradient(to_right,#bf953f,#fcf6ba,#b38728)1]">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h2 className="text-lg font-bold text-[#5c4a2e]">Escáner de bingo</h2>
